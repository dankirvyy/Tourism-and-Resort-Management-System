...
<head>
    <meta charset="UTF-8">
    <title>Book Tour: <?= html_escape($tour['name']) ?> - Visit Mindoro</title>
...
<body class="bg-gray-100">
    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden md:flex md:items-center md:space-x-4">
                ...
                <a href="<?= site_url('tours') ?>" class="font-semibold text-orange-600 px-3 py-2 rounded-md text-sm">Tours</a> <a href="<?= site_url('contact') ?>" class="text-gray-500 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium">Contact</a>
    ...
    <div class="md:hidden hidden" id="mobile-menu">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <a href="<?= site_url('tours') ?>" class="text-gray-700 bg-gray-100 block px-3 py-2 rounded-md text-base font-medium">Tours</a> <a href="<?= site_url('contact') ?>" class="text-gray-700 hover:bg-gray-50 block px-3 py-2 rounded-md text-base font-medium">Contact</a>
        </div>
    ...
    <div class="bg-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div>
                    <h2 class="text-3xl font-extrabold text-gray-900"><?= html_escape($tour['name']) ?></h2>
                    <p class="mt-4 text-lg text-gray-500"><?= html_escape($tour['description']) ?></p>
                    
                    <div class="mt-6">
                        <?php
                            $imageUrl = isset($tour['image_filename']) && $tour['image_filename']
                        ...
                        ?>
                        <img class="rounded-lg shadow-lg object-cover w-full h-80" src="<?= $imageUrl ?>" alt="<?= html_escape($tour['name']) ?>">
                    </div>
                    
                    <div class="mt-6 flex justify-between items-center">
                         <p class="text-2xl font-bold text-gray-900">₱<?= number_format($tour['price'], 2) ?> <span class="text-lg font-medium text-gray-500">/ person</span></p>
                         <p class="text-md text-gray-600">Duration: <?= html_escape($tour['duration']) ?></p>
                    </div>
                </div>

                <div class="bg-gray-50 p-8 rounded-lg shadow-lg">
                    <h3 class="text-2xl font-bold text-gray-900">Book This Tour</h3>
                    <form action="<?= site_url('book/process-tour') ?>" method="POST" class="mt-6 space-y-6">
                        <input type="hidden" name="tour_id" value="<?= $tour['id'] ?>">

                        <div>
                            <label for="booking_date" class="block text-sm font-medium text-gray-700">Select Date</label>
                            <input type="date" name="booking_date" id="booking_date" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                        </div>
                        <div>
                            <label for="num_pax" class="block text-sm font-medium text-gray-700">Number of People</label>
                            <input type="number" name="num_pax" id="num_pax" min="1" value="1" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                        </div>
