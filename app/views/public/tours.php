    ...
    <div class="bg-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                <h2 class="text-3xl font-extrabold text-gray-900">Explore Our Tours</h2>
                <p class="mt-4 text-lg text-gray-500">Experience the best of Mindoro with our guided adventures.</p>
            </div>
            
            <div class="mt-8 max-w-2xl mx-auto">
                <form action="<?= site_url('tours') ?>" method="GET" class="sm:flex sm:items-center sm:gap-4">
            ...
            <div class="mt-12 grid gap-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                <?php if (empty($tours)): ?>
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center text-gray-500">
                        <p class="text-xl">No tours found<?php if (!empty($search_term)): ?> matching "<?= html_escape($search_term) ?>"<?php endif; ?>.</p>
                        <a href="<?= site_url('tours') ?>" class="mt-2 text-orange-600 hover:text-orange-500">Clear search</a>
                    </div>
                <?php else: ?>
                    ...
                    <div class="flex flex-col rounded-lg shadow-lg overflow-hidden">
                        <div class="flex-shrink-0">
                            ...
                            ?>
                            <img class="h-56 w-full object-cover" src="<?= $imageUrl ?>" alt="<?= html_escape($tour['name']) ?>">
                        </div>
                        <div class="flex-1 bg-white p-6 flex flex-col justify-between">
                            <div class="flex-1">
                                <h3 class="text-xl font-semibold text-gray-900"><?= html_escape($tour['name']) ?></h3>
                                <p class="mt-3 text-base text-gray-500"><?= html_escape($tour['description']) ?></p>
                            </div>
                            <div class="mt-6">
                                <div class="flex items-center justify-between">
                                    <p class="text-lg font-medium text-gray-900">₱<?= number_format($tour['price'], 2) ?> <span class="text-sm text-gray-500">/ person</span></p>
                                    <p class="text-sm text-gray-500"><?= html_escape($tour['duration']) ?></p>
                                </div>
                                 <a href="<?= site_url('tour/' . $tour['id']) ?>" class="mt-4 block w-full text-center rounded-md border border-orange-600 bg-white px-4 py-2 text-base font-medium text-orange-600 shadow-sm hover:bg-orange-50">Learn More</a>
                                <a href="<?= site_url('book/tour/' . $tour['id']) ?>" class="mt-2 block w-full text-center rounded-md border border-transparent bg-orange-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-orange-700">Book Tour</a>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
